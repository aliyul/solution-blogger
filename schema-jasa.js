// ⚡ AUTO SCHEMA SERVICE v4.28+ FULL — Auto Detect Kabupaten/Kota/Kecamatan + Wikipedia Cache + Harga + ItemList
  ...
  setTimeout(async () => {
    ...
    const PAGE = {
      ...
      description: document.querySelector('meta[name="description"]')?.content?.trim() || 
                   document.querySelector("article p, main p, .post-body p")?.innerText?.substring(0, 200) || document.title,
    ...
    async function getCachedAreaList(cacheKey, areaName, type) {
      try {
        ...
        if(data?.length) {
          localStorage.setItem(cacheKey, JSON.stringify({expire: Date.now()+1000*60*60*24*30, data}));
          return data;
        }
    ...
    async function fetchAreaFromWikipedia(areaName,type){
      try {
        ...
        if(data?.parse?.text){
          const temp = document.createElement("div"); temp.innerHTML = data.parse.text["*"];
    ...
    // === 5️⃣ DETEKSI SERVICE & HARGA & ITEMLIST ===
    ...
    const detectInternalLinks = ()=>{
      return Array.from(document.querySelectorAll('article a, main a, .post-body a'))
    ...
    console.log(`[Schema Service v4.28+] ✅ Injected | ${serviceTypes.join(", ")} | Area: ${areaServed.length} | Links: ${internalLinks.length}`);
  },500);
});
