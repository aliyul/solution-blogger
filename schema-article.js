...
MONEY MASTER (L4) = "sewa excavator" (tanpa harga) → ENTITY MASTER

MONEY PAGE (L5) = "harga sewa excavator" (dengan harga) → COMMERCIAL SUPPORT

UPDATE v6.0:

PRIORITAS SEWA: tanpa "harga" = MONEY MASTER (L4), dengan "harga" = MONEY PAGE (L5)
...
Deteksi utama berdasarkan URL clean
...
if (/^\d{4}/.test(part)) return false; if (/^\d{2}/.test(part)) return false;
...
if (/(\d+(.\d+)?\smm\sx\s\d+(.\d+)?\smm\sx\s\d+(.\d+)?\smm)/i.test(lowerText)) score++;
if (/(\d+(.\d+)?\scm\sx\s\d+(.\d+)?\scm\sx\s\d+(.\d+)?\scm)/i.test(lowerText)) score++;
...
// 2. MONEY KEYWORDS (prioritas: cek "harga" dulu)
for (const kw of MONEY_KEYWORDS) {
...
// JASA: tidak ada money-master
if (isJasa) {
log(JASA → money-page (L5), "INFO");
return 'money-page';
}

// SEWA: aturan baru - jika mengandung "harga" → MONEY PAGE (L5)
if (isSewa) {
// Jika keyword adalah "harga" (bukan "sewa"), maka otomatis MONEY PAGE
if (kw === 'harga') {
log(SEWA dengan keyword "harga" → money-page (L5), "SUCCESS");
return 'money-page';
} else if (kw === 'sewa') {
// Jika keyword "sewa" (tanpa harga), maka gunakan wordCount
if (wordCount <= 2) {
...
// 8. SEWA (tanpa keyword harga - fallback)
if (isSewa) {
