...
(function AutoEvergreenV77Pillar(window, document) {
  ...
  try {
    ...
    if(elH1 && elH1.parentNode){ 
      ...
      label.style.fontSize='0.95em'; label.style.color='#222'; label.style.marginTop='6px'; label.style.marginBottom='12px'; 
      label.style.padding='6px 12px'; label.style.background='#f9f9f9'; label.style.border='1px solid #ccc'; 
      label.style.borderRadius='5px';
      label.innerHTML=`<b>${type}${isPillar?' (Pillar)':''}</b> — pembaruan berikutnya: <b>${nextUpdateStr}</b>`;
      elH1.parentNode.insertBefore(label, elH1.nextSibling);
    }
